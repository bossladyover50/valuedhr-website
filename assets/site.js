(function(){
  ...
  function showChatChoice(){
    ...
    button.type='button';button.className='chat-consent-launcher';button.textContent='Chat with us';
    button.setAttribute('aria-label','Enable live chat');
    var css=document.createElement('style');
    css.textContent='.chat-consent-launcher{position:fixed;z-index:9998;right:1rem;bottom:1rem;border:0;border-radius:999px;padding:.8rem 1.15rem;background:#2563eb;color:#fff;font:700 14px/1 system-ui;box-shadow:0 8px 24px #0003;cursor:pointer}.chat-consent-launcher:focus-visible{outline:3px solid #fbbf24;outline-offset:3px}';
    document.head.appendChild(css);document.body.appendChild(button);
    button.addEventListener('click',function(){localStorage.setItem('valuedhr-consent','accepted');button.remove();loadMarketing();});
  }
  var consent=localStorage.getItem('valuedhr-consent');
  if(consent==='accepted') loadMarketing();
  if(consent==='declined') showChatChoice();
  if(!consent){
    ...
    banner.addEventListener('click',function(e){var choice=e.target.dataset.consent;if(!choice)return;localStorage.setItem('valuedhr-consent',choice);banner.remove();if(choice==='accepted')loadMarketing();else showChatChoice();});
  }
})();
